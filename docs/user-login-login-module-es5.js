                      ...
                      .then(function (data) {
                        ...
                        _this2.storage.set('userData', JSON.stringify(_this2.userData)).subscribe({
                          next: function next() {
                            ...
                            _this2.fbStore.firestore.collection('users').doc(_this2.userData.uid).get().then(function (uData) {
                              // Se tem perfil
                              if (uData.exists) {
                                // Obtém dados do perfil
                                _this2.userProfile = uData.data();
                                _this2.userProfile.uid = uData.id; // Grava perfil no armazenamento local
                                ...
                                _this2.storage.set('userProfile', JSON.stringify(_this2.userProfile)).subscribe({
                                  next: function next() {
                                    ...
                                    _this2.events.publish('userProfile', _this2.userData); // Exibe alerta
